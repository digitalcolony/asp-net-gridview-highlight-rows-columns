/* ******************************************************************* *
 CSS/JavaScript Table Hilighting 0.2
 ---------------------------
 Author: Ryan Scherf 
...
var hilightTopHeader = 0; // Hilight top header cell			0,1
var hilightLeftHeader = 0; // Hilight leftmost header cell		0,1
...
function initCrossHairsTable(id, topPad, rightPad, bottomPad, leftPad) {
  ...
  for (var i = topPad; i < r; i++) {
    for (var j = leftPad; j < c; j++) {
      if (!document.all) {
        tableObj.rows[i].cells[j].setAttribute(
          "onmouseover",
          "addHilight(this);"
        );
        tableObj.rows[i].cells[j].setAttribute(
...
function applyHilightRightHeader(obj, rowIndex, colIndex, state) {
  ...
  if (state == 1) {
    ...
    classArray = obj.rows[rowIndex].cells[
      obj.rows[rowIndex].cells.length - 1
    ].className.split(" ");
    for (var i = 0; i < classArray.length - 1; i++)
      if (classArray[i] != "") rowClasses += " " + classArray[i];
    obj.rows[rowIndex].cells[
      obj.rows[rowIndex].cells.length - 1
  ...
  if (state == 1) {
    ...
    for (var i = 0; i < oldClasses.length; i++) {
      if (oldClasses[i] != hilightClass)
        obj.rows[rowIndex].cells[colIndex].className += " " + oldClasses[i];
...
function applySurroundActiveHilight(obj, rowIndex, colIndex, state) {
  ...
  if (state == 1) {
    for (
      var i = rowIndex - surroundRadius;
      i <= rowIndex + surroundRadius;
      i++
    ) {
      if (i < 0) continue;
      ...
      for (
        var j = colIndex - surroundRadius;
        j <= colIndex + surroundRadius;
        j++
      ) {
        if (j < 0) continue;
